...
def main(parser_data):
    ...
    val_dataset = VOCInstances(data_root, year="2007", txt_name="test.txt", transforms=data_transform["val"])
    ...
    backbone_with_fpn = BackboneWithFPN(new_backbone,
                                        ...
                                        re_getter=False)
    model = MaskRCNN(backbone_with_fpn, num_classes=args.num_classes + 1)
...
if __name__ == "__main__":
    ...
    # 训练好的权重文件
    parser.add_argument('--weights-path', default='./save_weights/wildfire-det.pth', type=str, help='training weights')

    # batch size(set to 1, don't change)
    parser.add_argument('--batch-size', default=1, type=int, metavar='N',
                        help='batch size when validation.')
    # 类别索引和类别名称对应关系
    parser.add_argument('--label-json-path', type=str, default="class.json")
